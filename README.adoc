...
This helps us determine the mutation score, which is killed / (killed + survived)
In subsequent runs, we will see the surviving mutations again.
Surviving mutations may indicate issues with our test set since application code changes should lead to different results, and thus to failing tests.
So to further reduce the survivors, we will need to enhance our test cases and/or add more tests.
...
Let's run `mvn test` on this project, and we'll get this magnificent JaCoCo report:

image::raw/jacoco.png[]

A line and branch coverage of 100% magnificent, isn't it?
...
Now that we know where the holes in our testing remain, we can work towards fixing these, and after these steps, we can rerun the Pitests to verify how many mutations remain alive.
...
For example, the second one, which makes sense given both `100 - 5 * 5` and `100 - 5 / 5` result in a number lower than 100.
...
For example, you can define which code is mutated, and which tests are run using it by using `targetClasses` and `targetTests` respectively.
...
Some common issues you'll see when first running Pitest are:

* assertions too weak (only “result < initial”)
* equivalent mutants (no observable behavior change)
...
One thinks they're done, but not quite. It might make one grumble a bit, and add some test cases to satisfy the tool, which is the wrong motivator.
